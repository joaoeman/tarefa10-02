...
void ssd1306_draw_square(uint8_t *ssd, int x, int y, bool set) {
    // As coordenadas dos 4 cantos do quadrado de 8x8 pixels
    int x0 = x, y0 = y;            // Canto superior esquerdo
    int x1 = x + 7, y1 = y;        // Canto superior direito
    int x2 = x + 7, y2 = y + 7;    // Canto inferior direito
    int x3 = x, y3 = y + 7;        // Canto inferior esquerdo

    // Desenha as 4 linhas para formar o quadrado
    ssd1306_draw_line(ssd, x0, y0, x1, y1, set);  // Linha superior
    ssd1306_draw_line(ssd, x1, y1, x2, y2, set);  // Linha direita
    ssd1306_draw_line(ssd, x2, y2, x3, y3, set);  // Linha inferior
    ssd1306_draw_line(ssd, x3, y3, x0, y0, set);  // Linha esquerda
}
